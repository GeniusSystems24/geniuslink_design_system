<!--
  ...
  Component: ReadableTable
  Scope: read this before writing or editing Flutter code that uses ReadableTable from
  the geniuslink_design_system package. It is the authoritative how-to-use guide.
-->

# Codex skill — GeniusLink ReadableTable

**When this applies.** You are working in a Flutter project that depends on
`geniuslink_design_system` and the task involves ReadableTable — a read-only, generic typed-row display grid with selection, click-to-sort, keyboard nav, and a full filter system (flat chips, nested query builder, inline column filters).
