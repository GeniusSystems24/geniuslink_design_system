<!--
  ...
  Scope: read this before writing or editing Flutter code that uses EditableTable from
  the geniuslink_design_system package. It is the authoritative how-to-use guide.
-->

# Codex skill — GeniusLink EditableTable

**When this applies.** You are working in a Flutter project that depends on
`geniuslink_design_system` and the task involves EditableTable — an Excel-style, typed-column data-entry grid (inline editing, validation, totals, column resize/reorder, TSV copy, undo/redo, RTL).
