<!--
  ...
  Component: AutoSuggestionsBox
  Scope: read this before writing or editing Flutter code that uses AutoSuggestionsBox from
  the geniuslink_design_system package. It is the authoritative how-to-use guide.
-->

# Codex skill — GeniusLink AutoSuggestionsBox

**When this applies.** You are working in a Flutter project that depends on
`geniuslink_design_system` and the task involves AutoSuggestionsBox — a typed auto-suggest / combo field with static / async / hybrid sources, match highlighting, multi-select, custom builders, and an embeddable bare mode.
