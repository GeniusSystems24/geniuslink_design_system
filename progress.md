...
7. **Forms** — implement ComboBox with MVVM view model for single, multi, async, searchable, empty, loading, and error states.
...
| `components-combobox.html` | `lib/design_system/components/forms/combo_box.dart` | MVVM | Complete | 1.1.6 |
...
| `GLComboOption`, `GLComboBoxViewModel`, `GLComboBox` | Complete |
...
- The current package version is `1.2.0`. The changelog records release checkpoints for each completed component group.
- I could not run `flutter analyze` in this sandbox because Flutter/Dart tooling is not installed here; the code is organized and checked manually against Dart syntax and public exports.
